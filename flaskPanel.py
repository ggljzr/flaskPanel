...
def parse_ifconfig(interface):
    ...
    # try to find hwaddr
    proc_ifconfig = subprocess.Popen(
        ['ifconfig', str(interface)], stdout=subprocess.PIPE)
    # shell=True for trusted input only (shell injection)
    try:
        hwaddr = subprocess.check_output(
            ['grep', 'HWaddr'], stdin=proc_ifconfig.stdout)
    ...
    proc_ifconfig = subprocess.Popen(
        ['ifconfig', str(interface)], stdout=subprocess.PIPE)
    try:
        inet_row = subprocess.check_output(
            ['grep', 'inet'], stdin=proc_ifconfig.stdout).strip().split()
    except subprocess.CalledProcessError:
...
def parse_df(device):
    # return os.popen('df -h ' + str(device) + ' | tail -n 1',
    # 'r').read().split()

    proc_df = subprocess.Popen(
        ['df', '-h', '-T', str(device)], stdout=subprocess.PIPE)
    # device info, skip device name
    dev_info = subprocess.check_output(
        ['tail', '-n', '1'], stdin=proc_df.stdout).split()[1:]
    ...
    if len(dev_info) > 4:
        ...
        dev_info[2] = "Used: " + dev_info[2] + \
            " (" + dev_info[4] + ")"  # used space + percentage
...
def hello_world():
    ...
    # tuples in formate (device name(string), device info (list of strings))
    devices = []
...
def get_theme():
    try:
        proc = subprocess.check_output(
            'cat static/style.css | grep name:', shell=True)
    except subprocess.CalledProcessError:
